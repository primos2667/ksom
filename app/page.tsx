...
function NotificationBellInstagram({ isDark, onNotification }: { isDark: boolean; onNotification?: () => void }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('ksom-notif-v9')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'products' }, (payload) => {
        const newNotif = {
          id: Date.now().toString(),
          type: "product",
          title: "New product on KSOM",
          message: `${payload.new.title} • ${payload.new.price}`,
  ...
  return (
    <div className="relative">
      <button
        ...
        className={`relative w-10 h-10 rounded-full grid place-items-center backdrop-blur border transition-all active:scale-90 ${isDark ? "bg-white/10 border-white/20 text-white hover:bg-white/15" : "bg-black/5 border-black/10 text-black hover:bg-black/10"}`}
      >
        {/* Instagram / YouTube style heart / bell icon - Clean outline */}
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
          <path d="M6 8a6 6 0 0 1 12 0c0 7 6 9 6 9H0s6-2 6-9"></path>
          <path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"></path>
        </svg>
        {hasNew && unread > 0 && (
          <>
            <span className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse border-2 border-white/20"></span>
            <span className="absolute -top-1 -right-1 min-w-[16px] h-4 bg-red-500 text-white text-[9px] rounded-full grid place-items-center px-1 font-bold sm:hidden">{unread > 9 ? "9+" : unread}</span>
          </>
        )}
      </button>
      {show && (
          ...
          <div className="overflow-y-auto max-h-[360px]">
            {notifs.length === 0 ? (
              <div className="p-8 text-center">
                <div className={`w-12 h-12 rounded-full mx-auto grid place-items-center mb-3 ${isDark ? "bg-white/5" : "bg-black/5"}`}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className={isDark ? "text-white/40" : "text-black/30"}><path d="M6 8a6 6 0 0 1 12 0c0 7 6 9 6 9H0s6-2 6-9"></path></svg>
                </div>
                <p className={`text-[12px] font-medium ${isDark ? "text-white/60" : "text-black/50"}`}>No notifications yet</p>
                <p className={`text-[10px] mt-1 ${isDark ? "text-white/30" : "text-black/30"}`}>When someone posts a new product, you'll see it here</p>
              </div>
            ) : notifs.map((n: any) => (
              <div key={n.id} className={`p-3.5 flex gap-3 border-b last:border-0 hover:bg-white/[0.03] transition-colors ${isDark ? "border-white/5" : "border-black/5"} ${!n.read ? (isDark ? "bg-white/[0.04]" : "bg-black/[0.02]") : ""}`}>
                {n.image ? <img src={n.image} className="w-10 h-10 rounded-full object-cover shrink-0" alt="" /> : <div className={`w-10 h-10 rounded-full grid place-items-center shrink-0 ${isDark ? "bg-white/10" : "bg-black/5"}`}>🛒</div>}
                <div className="flex-1 min-w-0">
                  <p className={`text-[12px] leading-tight ${isDark ? "text-white" : "text-black"}`}><span className="font-bold">{n.title}</span> • {n.message}</p>
                  <p className={`text-[10px] mt-1 ${isDark ? "text-white/40" : "text-black/40"}`}>{new Date(n.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} ago</p>
                </div>
                {!n.read && <div className="w-2 h-2 bg-[#0d9488] rounded-full shrink-0 mt-2"></div>}
              </div>
            ))}
          </div>
        </div>
...
export default function HomeFinal() {
  ...
  return (
    <div className={`${isDark ? "bg-[#0f0f0f] text-[#f5f3ef]" : "bg-[#fbfaf8] text-[#121212]"} min-h-screen pb-28 transition-colors`}>
      ...
      {/* NEW HOMEBAR - Home + Bell + Cart + Theme - NO FAVORITE - Instagram/YouTube style bell */}
      <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50">
        <div className={`flex items-center gap-1 rounded-full p-1.5 backdrop-blur-[28px] border shadow-[0_12px_32px_rgba(0,0,0,0.15)] ${isDark ? "bg-[#1e1e1e]/80 border-white/10" : "bg-white/80 border-black/10"}`}>
          <a href="/" className={`flex items-center gap-2 px-6 py-2.5 rounded-full text-[12px] font-medium shadow-sm transition-all ${isDark ? "bg-white text-black" : "bg-black text-white"}`}>
          ...
          <NotificationBellInstagram isDark={isDark} onNotification={() => { setNotifPulse(true); setTimeout(() => setNotifPulse(false), 1000); }} />

          <a href="/cart" className={`w-10 h-10 rounded-full grid place-items-center backdrop-blur relative border transition-all active:scale-90 ${isDark ? "bg-white/10 text-white border-white/10 hover:bg-white/15" : "bg-black/5 text-black border-black/5 hover:bg-black/10"} ${notifPulse ? "animate-pulse" : ""}`}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="9" cy="21" r="1"></circle><circle cx="20" cy="21" r="1"></circle><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"></path></svg>
            {cart.length > 0 && <span className="absolute -top-1 -right-1 min-w-[16px] h-4 bg-black text-white text-[9px] rounded-full grid place-items-center px-1 font-bold border border-white/20">{cart.length}</span>}
          </a>
